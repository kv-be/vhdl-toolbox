...
If the tool finds (assumed) parser errors, it stops parsing the file and shows the parser error. All files containing parsing errors are marked after parsing in the file list and problems view.
...
|  CTRL+ALT+e      | detects a case statement in the selected text and 
   - defines a type based on the case signal containing all enums in the case
   - defined the case signal as a signal of the type above
|
