...
function connect_axi(name, text) {
    let max = 0
    let pos = 0

    const mosi = ["araddr", "arvalid", "arid", "arlen", "arsize", "arburst", "arlock", "arcache", "arprot", "arqos", "rready", "awaddr", "awvalid", "awid", "awlock", "awlen", "awsize", "awburst", "awcache", "awqos", "awprot", "wdata", "wvalid", "wid", "wlast", "wstrb", "bready"]
    const miso = ["arready", "rdata", "rvalid", "rlast", "rid", "rresp", "awready", "wready", "bresp", "bvalid", "bid",]
    var master_name
    var slave_name
    var not_found_miso = []
    var not_found_mosi = []
    if (name.indexOf("_mosi") > -1) {
        master_name = name
        slave_name = name.replace("mosi", "miso")
    }
    if (name.indexOf("miso") > -1) {
        master_name = name.replace("miso", "mosi")
        slave_name = name
    }
    for (var i = 0; i < miso.length; i++) {
        var _regex = new RegExp("([ \t]*.*_" + miso[i] + "[ \t]*=>).*", "gi")
        if (!text.match(_regex)) {
            not_found_miso.push(slave_name + "." + miso[i]);
        }
        text = text.replace(_regex, "$1 " + slave_name + "." + miso[i] + ",")
    }
    for (var i = 0; i < mosi.length; i++) {
        var _regex = new RegExp("([ \t]*.*_" + mosi[i] + "[ \t]*=>).*", "gi")
        if (!text.match(_regex)) {
            not_found_mosi.push(master_name + "." + mosi[i]);
        }
        text = text.replace(_regex, "$1 " + master_name + "." + mosi[i] + ",")
    }
    text = text + "\n-- the following miso signals are not assigned in the port map"
    for (i = 0; i < not_found_miso.length; i++) {
        text = text + "\n--" + not_found_miso[i]
    }
    text = text + "\n-- the following mosi signals are not assigned in the port map"
    for (i = 0; i < not_found_mosi.length; i++) {
        text = text + "\n--" + not_found_mosi[i]
    }

    return text
}
exports.connect_axi = connect_axi
